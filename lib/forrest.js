...
var Forrest = CTS.Forrest = function(opts) {
  ...
  this.defaultTreeReady = CTS.Promise.defer();
...
CTS.Fn.extend(Forrest.prototype, {
  ...
  removeRelationSpec: function(spec) {
    this.relationSpecs = CTS.Fn.filter(this.relationSpecs,
  ...
  reloadTreeSpec: function(name, render) {
    ...
    this.realizeTree(spec).then(
     function(tree) {
      ...
      CTS.Log.Info("Re-Realized tree", name);
  ...
  addSpec: function(forrestSpec) {
    ...
    if (typeof forrestSpec.treeSpecs != 'undefined') {
      ...
      CTS.Promise.all(promises).then(function() {
        initial.resolve();
      });
