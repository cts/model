...
//      }
   ...
   * then add.
   */
  realizeRelations: function(subtree, butOnto, filterFn) {
    for (var i = 0; i < this.relationSpecs.length; i++) {
      this.realizeRelation(this.relationSpecs[i], subtree, butOnto, filterFn);
    }
  },
   ...
   *
   */
  realizeRelation: function(spec, subtree, butOnto, filterFn) {
        ...
        if ((!subtree) ||
          ...
          if ((!filterFn) || (filterFn({node1: node1, node2: node2}))) {
            if (filterFn) {
              ...
              console.log("passed filter", node1, node2);
            }
            if (subtree && butOnto) {
