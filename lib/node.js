...
Node.Base = {
  ...
  containsRelation: function(relation) {
    ...
    if (Util.Fn.contains(this.relations, relation)) {
      return true;
    }
  ...
  unregisterRelation: function(relation) {
    ...
    this.relations = Util.Fn.filter(this.relations,
  ...
  markRelationsAsForCreation: function(val, recurse, insideOtherSubtree) {
    ...
    for (var i = 0; i < rs.length; i++) {
      ...
      console.log("Mark Reln", this.ctsId, (insideOtherSubtree.ctsId || null), rs[i]._forCreationOnly, val);
      if (insideOtherSubtree) {
        if ((rs[i].node1 == this) && (rs[i].node2.isDescendantOf(insideOtherSubtree))) {
          rs[i].forCreationOnly(val);          
          console.log("Create1", rs[i], this, val);
        } else if ((rs[i].node2 == this) && (rs[i].node1.isDescendantOf(insideOtherSubtree))) {
          rs[i].forCreationOnly(val);
          console.log("Create2", rs[i], this, val);
        }
      } else {
        rs[i].forCreationOnly(val);
        console.log("Create3", rs[i], this, val);
      }
    }
    if (recurse) {
  ...
  parseInlineRelationSpecsRecursive: function() {
    ...
    this.parseInlineRelationSpecs().then(
      function() {
        Util.Promise.all(Util.Fn.map(self.children, function(kid) {
  ...
  getSubtreeRelations: function() {
    return Util.Fn.union(this.getRelations(), Util.Fn.flatten(
      Util.Fn.map(this.getChildren(), function(kid) {
    ...
    this.getRelations().then(function(relations) {
      var kidPromises = Util.Fn.map(this.getChildren(), function(kid) {
        return kid.getSubtreeRelations();
      });
      if (kidPromises.length == 0) {
        ...
        Q.allSettled(kidPromises).then(function(results) {
          ...
          if (!rejected) {
            var allR = Util.Fn.union(relations, Util.Fn.flatten(kidRelations));
    ...
    if (this.parentNode) {
      for (var i = 0; i < this.parentNode.children.length; i++) {
        if (this.parentNode.children[i] == this) {
          Util.Fn.arrDelete(this.parentNode.children, i, i);
          gotIt = true;
          break;
        }
      }
  ...
  realizeChildren: function() {
    ...
    sc.then(
      function() {
        var promises = Util.Fn.map(self.children, function(child) {
  ...
  pruneRelations: function(otherParent, otherContainer) {
    var self = this;
    this.relations = Util.Fn.filter(this.getRelations(), function(r) {
  ...
  _processIncoming: function(insideOtherSubtree) {
    ...
    self._processIncomingRelations(r, 'is', insideOtherSubtree, false, true).then(function() {
      ...
      return Util.Promise.all(Util.Fn.map(self.getChildren(), function(child) {
