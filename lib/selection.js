...
Util.Fn.extend(Selection.prototype, {
  contains: function(node) {
    return Util.Fn.contains(this.nodes, node);
  },

  clone: function() {
    // not a deep clone of the selection. we don't want duplicate nodes
    // running around.
    return new Selection(Util.Fn.union([], this.nodes), this.opts);
  },
  ...
  matchesArray: function(arr, exactly, orArrayAncestor) {
    ...
    for (var i = 0; i < this.nodes.length; i++) {
      if (! Util.Fn.contains(arr, this.nodes[i])) {
