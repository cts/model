...
Util._.extend(Selection.prototype, {
  ...
  render: function(insideOtherSubtree, opts, root) {
    if ((typeof this.nodes != 'undefined') && (this.nodes.length )) {
      return _.map(this.nodes, function(n) {n._processIncoming(insideOtherSubtree, opts, root)});
    }
  },

  editable: function(val) {
    if (val === true) {
      if (CTS.engine && CTS.engine.UI) {
        _.map(this.nodes, function(n) {
          n._editable = new CTS.engine.UI.Editable(n.value);
        });
      }
    }
  },
