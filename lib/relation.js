...
Relation.Base = {
  ...
  handleEventFromNode: function(evt, relationChain) {
    ...
    if (thrownTo.shouldReceiveEvents) {
      ...
      if (typeof relationChain == 'undefined') {
        relationChain = [evt];
      } else {
        relationChain.push(evt);
      }
      for (var i = 0; i < otherRelations.length; i++) {
        if (otherRelations[i] != this) {
          var newEvent = Util._.clone(evt);
          newEvent.sourceNode = thrownTo;
          newEvent.sourceTree = thrownTo.tree;
          newEvent.ctsNode = thrownTo;
          otherRelations[i].handleEventFromNode(newEvent, relationChain);
        }
      }
    }
  },
