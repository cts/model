...
Relation.Base = {
  ...
  _cloneIterable: function(toward, cloneFromIndex, insertAfterIndex, throwEvent, beforePersistenceFn, cloneRelns) {
    ...
    var cloned = function(clone) {
      ...
      clone.realizeInlineRelationSpecs(true, filterFn1);
      // And now for external rules  
      toward.tree.forrest.realizeRelations(clone, filterFn1);
    ...
    iterables[cloneFromIndex].clone(beforePersistenceFn, cloneRelns).then(
      cloned,
      function(err) {
        deferred.reject(err);
      }
